...
class Spider(Spider):
    ...
    def detailContent(self, array):
        ...
        for video in vodList:
            # 优先使用 pid，如果没有则使用 guid（部分视频 pid 可能为空）
            play_id = video.get('pid') or video.get('guid')
            if play_id:
                videoList.append(video['title'] + "$" + play_id)
        if len(videoList) == 0:
            return {}
        if len(date) == 0:
    ...
    def _get_raw_hls_url(self, pid):
        ...
        try:
            ...
            if data and "hls_url" in data:
                hls_url = data["hls_url"].strip()
                self._cache[cache_key] = hls_url
                return hls_url
        except Exception as e:
            print(f"获取播放地址失败 pid={pid}: {e}")
        return None
    
    def playerContent(self, flag, id, vipFlags):
        """获取播放地址，返回原始 hls_url"""
        result = {}
        # id 是从播放列表中取出的 pid（或 guid）
        raw_url = self._get_raw_hls_url(id)
        if raw_url:
            result["parse"] = 0
            result["playUrl"] = ""
            result["url"] = raw_url
            result["header"] = self.header   # 可选的请求头
        else:
            # 降级：返回原始 id（虽然可能无法播放，但避免崩溃）
            result["parse"] = 0
            result["playUrl"] = ""
            result["url"] = id
            result["header"] = ""
